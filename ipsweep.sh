...
function display_help() {
    ...
    echo "ipsweep 192.168.1"
    echo
    echo "MAKE SURE TO ASK FOR PERMISSION. IT IS ILLEGAL IF PERMISSION WAS 
NOT GRANTED"
}
...
function display_ip_info() {
    ip a
    echo "To scan other IPs, copy the first 3 octets of your IP. For 
example: 192.168.1"
...
for ip in $(seq 1 254); do
    if ping -c 1 "$1.$ip" | grep "64 bytes"; then
        ip_address=$(echo "$_" | cut -d " " -f 4 | tr -d ":")
...
echo "Would you like to store these IPs in a temporary file? y[es] or 
n[o]"
read answer
...
if [[ $nmap_answer =~ ^[Yy][Ee][Ss]|[Yy]$ ]]; then
    ...
    echo "$nmap_command"
    echo "Would you like to execute this command line or paste it 
manually? e[xecute] or m[anual scan]"
    read execute_answer
